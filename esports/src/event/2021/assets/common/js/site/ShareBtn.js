export default class ShareBtn {
  ...
  setLine(data) {
    for (let i = 0; i < this.line.length; i += 1) {
      ...
      if (this.isPC) {
        encodedShareText = `https://social-plugins.line.me/lineit/share?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`;
      } else {
        encodedShareText = `http://line.me/R/msg/text/?${encodeURIComponent(text + url)}`;
      }
      this.line[i].href = encodedShareText;
